...
public final class AddTask implements Command {
    ...
    public AddTask(final DataBaseLinker inputLinker, final Reactor reactorArg) {
        this.db = inputLinker;
    ...
    public String getHelp() {
        return "Creates new task.\n"
            + "Syntax: /add <name> <date>";
    }
        ...
        try {
            ...
            if (db.getUserById(userId).getLimit() == 0) {
                NumberedString answer = new NumberedString(userId, "You have used up all your "
                    + "task cells, please close one or more tasks before add a new one.");
