...
public class Postpone extends ListPersonalTasks {
    private final Reactor reactor;
    private final SQLiteLinker db;
    private static final int SECONDS_IN_DAY = 86400;


    public Postpone(final SQLiteLinker inputLinker, final Reactor reactorArg) {
        super(inputLinker);
        this.db = inputLinker;
    ...
    public final String getHelp() {
        return "Sets notification time for chosen task.\nInteractive mode only.";
    }
    ...
    public final State execute(final int userId, final String[] args, final State state,
        ...
        final BlockingQueue<NumberedString> responseQueue) throws CommandException {
            if (args.length != 0 && isValidId(args[0], currentState.getMemory().length)) {
                ...
                NumberedString answer = new NumberedString(userId,
                        "Please enter a valid deadline id");
                responseQueue.add(answer);
